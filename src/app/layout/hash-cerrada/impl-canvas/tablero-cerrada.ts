export class TableroCerrada{
    ...
    agregarNodo(numIndice:number, numSubIndice:number, valor:string){
        ...
        var separacion=50;
        this.ctx.moveTo( this.listaCoordenadasX[numIndice]+2, this.listaCoordenadasY[numIndice]+1 );
        this.ctx.lineTo( this.listaCoordenadasX[numIndice]+this.anchoNodo-2, this.listaCoordenadasY[numIndice]+1);
        this.ctx.lineTo( this.listaCoordenadasX[numIndice]+this.anchoNodo-2, this.listaCoordenadasY[numIndice]+this.altoNodo-1 );
        this.ctx.lineTo( this.listaCoordenadasX[numIndice]+2, this.listaCoordenadasY[numIndice]+this.altoNodo-1);
        this.ctx.lineTo( this.listaCoordenadasX[numIndice]+2, this.listaCoordenadasY[numIndice]+1 );
        this.ctx.fillText( valor, this.listaCoordenadasX[numIndice]+15+separacion, this.listaCoordenadasY[numIndice]+30 );
        this.ctx.stroke();
    }
    ...
    mostrarMensajeEncontrado(valor:string,encontrada:number){
        var textoEncontrado="No encontrado";
        if(encontrada>=0) textoEncontrado="Encontrado";
        this.ctx.fillStyle = this.colorLineas;
        this.ctx.fillText("Valor "+valor+" "+textoEncontrado+(encontrada>=0?(" posición: "+encontrada):""), this.ctx.canvas.width/2,40);
    }
    borrarSelecciones(){
